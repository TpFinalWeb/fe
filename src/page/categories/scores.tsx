...
function Scores() {
  ...
  useEffect(() => {
      ...
      const fetchDataGOY = async () => {
        ...
        (acc, item) => {
          if (item.top_score < 1) {
            ...
            acc.platforms.push(item.top_name);
    ...
    async function handlePlatformOptions(platform_name: string) {
      try {
      const data = await GraphService.getPlatformQualityByTime(platform_name);
      setDataset(data.aggregation.map((game: any) => game.average_score));
      setLabels(data.aggregation.map((game: any) => game.platform_name));
      let colorsArray: string[] = [];
    ...
    async function handleOption(params: string) {
      ...
      const data = await GraphService.getGenreQualityByTime(params)
      setDataset1(data.aggregation.map((game: any) => game.average_score));
      setLabels1(data.aggregation.map((game: any) => game.genre_name));
      let colorsArray: string[] = [];
      for (let i = 0; i < data.aggregation.length; i++) {
        const letters = '0123456789ABCDEF';
        let color = '#';
        ...
        <main className="flex-1 bg-teal-50 flex flex-col items-center justify-center p-8">
          ...
          <div className="flex flex-wrap justify-center gap-12 mt-10">
            <div className="bg-white shadow-md rounded-lg p-10 w-[500px] hover:scale-105 hover:shadow-lg cursor-pointer">
              ...
              <Bar data={dataPlatform} options={optionsGenre} />
              <button 
